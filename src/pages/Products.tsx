...
export function Products() {
  ...
  const toggleProduct = async (id: string) => {
    ...
    try {
      ...
      setApiError(error instanceof Error ? error.message : "No fue posible actualizar el producto.");
  ...
  const openEdit = (product: Product) => {
    ...
    const variant = product.variants[0];
    if (!variant) return;
    const weightUnit = variant.weightUnit === "KILOGRAMS" ? "kg" : "g";
    setEditing({ productId: product.id, variantId: variant.id });
    setDraft({ title: product.title, sku: variant.sku, price: variant.price, weight: variant.weight, weightUnit, packageLengthCm: variant.packageLengthCm, packageWidthCm: variant.packageWidthCm, packageHeightCm: variant.packageHeightCm });
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      setApiError(error instanceof Error ? error.message : "No fue posible guardar el producto en Shopify.");
    } finally { setSaving(false); }
  };
