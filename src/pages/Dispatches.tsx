...
export function Dispatches() {
  ...
  return <section className="panel full-panel">
    ...
    {selected && <div className="modal-backdrop"><div className="modal-card"><p className="eyebrow">Detalle de despacho</p><h2>{selected.id}</h2><p className="muted">{selected.customer}{selected.email ? ` · ${selected.email}` : ""}</p><div className="detail-grid"><div><small className="muted">Estado</small><strong>{dispatchStatus(selected) || "Pendiente"}</strong></div><div><small className="muted">Fulfillment Shopify</small><strong>{selected.fulfillmentStatusDetail || selected.fulfillmentStatus || "Pendiente"}</strong></div></div>{canManage && selected.status !== "Enviado" && selected.status !== "Entregado" && <div className="fulfillment-form"><h3 className="modal-section-title">Crear despacho en Shopify</h3><div className="form-grid"><label>Transportista<input value={carrier} onChange={(e) => setCarrier(e.target.value)} placeholder="Ej. Blue Express" /></label><label>Número de seguimiento<input value={tracking} onChange={(e) => setTracking(e.target.value)} placeholder="Opcional" /></label><label className="form-grid-wide">URL de seguimiento<input value={trackingUrl} onChange={(e) => setTrackingUrl(e.target.value)} placeholder="https://..." /></label></div><button className="primary-button" disabled={saving} onClick={markShipped}>{saving ? "Creando despacho…" : "Marcar enviado en Shopify"}</button></div>}{selected.trackingUrl && <a className="primary-button" href={selected.trackingUrl} target="_blank" rel="noreferrer">Ver seguimiento</a>}<div className="modal-actions"><button className="action-button" onClick={() => setSelected(null)}>Cerrar</button></div></div></div>}
  </section>;
}
