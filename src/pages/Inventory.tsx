...
export function Inventory() {
  ...
  const openAdjustment = (productId: string, variantId: string) => { setEditing({ productId, variantId }); setAmount(1); setReason("Reposición"); };
  ...
  return <section className="panel full-panel">
    ...
    <ProductTable products={filteredProducts} showWeight actions={{ onToggle: (id) => { const p = products.find((x) => x.id === id); if (p?.variants.length) openAdjustment(p.id, p.variants[0].id); }, onEdit: (product) => { if (product.variants.length) openAdjustment(product.id, product.variants[0].id); } }} />
