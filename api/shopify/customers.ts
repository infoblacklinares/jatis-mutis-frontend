...
type CustomersConnection = { nodes: CustomerNode[]; pageInfo: { hasNextPage: boolean; endCursor: string | null } };
type ShopifyResponse = { data?: { customers?: CustomersConnection }; errors?: Array<{ message?: string }> };
...
const query = `query Customers($first: Int!, $after: String) { customers(first: $first, after: $after, sortKey: NAME) { nodes { id displayName firstName lastName defaultEmailAddress { emailAddress } defaultPhoneNumber { phoneNumber } numberOfOrders amountSpent { amount currencyCode } state createdAt updatedAt lastOrder { id createdAt name } } pageInfo { hasNextPage endCursor } } }`;

async function graphql(shop: string, token: string, variables: Record<string, unknown>) {
  const response = await fetch(`https://${shop}/admin/api/${SHOPIFY_API_VERSION}/graphql.json`, { method: "POST", headers: { "Content-Type": "application/json", "X-Shopify-Access-Token": token }, body: JSON.stringify({ query, variables }) });
  const payload = await response.json() as ShopifyResponse; const details = payload.errors?.map((error) => error.message || "Error GraphQL").join("; ") || "sin detalle";
  if (!response.ok || payload.errors?.length) throw new Error(`GRAPHQL_${response.status}:${details}`); if (!payload.data?.customers) throw new Error("GRAPHQL_CUSTOMERS_EMPTY"); return payload.data.customers;
}

function normalize(customer: CustomerNode) {
  return {
    id: customer.id,
    name: customer.displayName || customer.email || customer.phone || "Cliente sin nombre",
    email: customer.defaultEmailAddress?.emailAddress || "Sin correo",
    phone: customer.defaultPhoneNumber?.phoneNumber || "Sin teléfono",
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const authorization = String(req.headers.authorization || ""); const customIdToken = String(req.headers["x-shopify-id-token"] || ""); const idToken = customIdToken || (authorization.startsWith("Bearer ") ? authorization.slice(7) : "");
    ...
    const nodes: CustomerNode[] = []; let after: string | null = null; let hasNextPage = true;
    while (hasNextPage) { const page = await graphql(shop, token, { first: 100, after }); nodes.push(...page.nodes); hasNextPage = page.pageInfo.hasNextPage; after = page.pageInfo.endCursor; }
