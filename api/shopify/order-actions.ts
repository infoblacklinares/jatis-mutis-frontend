...
async function graphql(shop: string, token: string, query: string, variables: Record<string, unknown>) {
  ...
  const payload = await response.json() as { data?: Record<string, any>; errors?: Array<{ message?: string }> };
...
async function fulfillOrder(shop: string, token: string, orderId: string, tracking?: { company?: string; number?: string; url?: string }, notifyCustomer = false) {
  ...
  const lineItemsByFulfillmentOrder = open.map((node) => ({ fulfillmentOrderId: node.id, fulfillmentOrderLineItems: node.lineItems.nodes.filter((item) => item.remainingQuantity > 0).map((item) => ({ id: item.id, quantity: item.remainingQuantity })) })).filter((item) => item.fulfillmentOrderLineItems.length);
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const body = (req.body || {}) as { orderId?: string; action?: string; tracking?: { company?: string; number?: string; url?: string }; notifyCustomer?: boolean; restock?: boolean; staffNote?: string };
    ...
    console.error(error); return json(res, 502, { error: "No fue posible actualizar el pedido en Shopify.", detail: message });
  }
}
