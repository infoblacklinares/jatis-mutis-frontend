...
type ShopifyOrder = {
  ...
  fulfillments: Array<{ id: string; status: string; displayStatus: string | null; trackingInfo: Array<{ company: string | null; number: string | null; url: string | null }> }>;
...
const query = `query Orders($first: Int!, $after: String) { orders(first: $first, after: $after, sortKey: CREATED_AT, reverse: true) { nodes { id name createdAt displayFinancialStatus displayFulfillmentStatus totalPriceSet { shopMoney { amount currencyCode } } customer { displayName email } fulfillments { id status displayStatus trackingInfo { company number url } } lineItems(first: 100) { nodes { quantity name sku originalUnitPriceSet { shopMoney { amount } } variant { id sku product { id title } } } } } pageInfo { hasNextPage endCursor } } }`;
