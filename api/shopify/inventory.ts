...
function verify(token: string, clientId: string, secret: string) {
  ...
  const [h, p, s] = parts; const header = JSON.parse(decode(h).toString()) as { alg?: string }; const claims = JSON.parse(decode(p).toString()) as Claims;
  ...
  const now = Math.floor(Date.now() / 1000); if (!claims.exp || claims.exp <= now) throw new Error("SHOPIFY_ID_TOKEN_EXPIRED");
  if (claims.nbf && claims.nbf > now) throw new Error("SHOPIFY_ID_TOKEN_NOT_ACTIVE"); if (claims.aud !== clientId) throw new Error("SHOPIFY_ID_TOKEN_AUDIENCE_INVALID");
...
async function requireAdmin(req: VercelRequest) {
  const request = new Request(requestUrl(req), { method: req.method || "GET", headers: new Headers(req.headers as Record<string, string>), body: req.method === "GET" || req.method === "HEAD" ? undefined : JSON.stringify(req.body ?? {}) });
  const state = await clerk.authenticateRequest(request, { authorizedParties: (process.env.CLERK_AUTHORIZED_PARTIES || process.env.APP_URL || "").split(",").map((v) => v.trim()).filter(Boolean) });
  if (!state.isAuthenticated) throw new Error("UNAUTHORIZED"); const auth = state.toAuth(); if (!auth.userId) throw new Error("UNAUTHORIZED");
  const user = await clerk.users.getUser(auth.userId); if (user.publicMetadata?.role !== "Administrador") throw new Error("FORBIDDEN");
}
...
async function graphql(shop: string, token: string, query: string, variables: Record<string, unknown>) {
  ...
  const payload = await response.json() as { data?: any; errors?: Array<{ message?: string }> }; const details = payload.errors?.map((e) => e.message || "Error GraphQL").join("; ") || "sin detalle";
  if (!response.ok || payload.errors?.length) throw new Error(`GRAPHQL_${response.status}:${details}`); return payload.data;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const clientId = process.env.SHOPIFY_CLIENT_ID || ""; const secret = process.env.SHOPIFY_CLIENT_SECRET || ""; const auth = String(req.headers.authorization || "");
    ...
    const { inventoryItemId, locationId, delta, reason = "correction", referenceDocumentUri } = req.body || {};
    ...
    const errors = data.inventoryAdjustQuantities?.userErrors || []; if (errors.length) return json(res, 422, { error: "Shopify rechazó el ajuste.", errors });
    return json(res, 200, { success: true, adjustment: data.inventoryAdjustQuantities?.inventoryAdjustmentGroup || null });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Error interno"; if (message === "UNAUTHORIZED") return json(res, 401, { error: "No autenticado." }); if (message === "FORBIDDEN") return json(res, 403, { error: "Se requiere rol Administrador." });
    if (message.startsWith("SHOPIFY_ID_TOKEN_")) return json(res, 401, { error: "Sesión de Shopify inválida o expirada.", detail: message }); console.error(error); return json(res, 502, { error: "No fue posible ajustar el inventario en Shopify.", detail: message });
  }
}
