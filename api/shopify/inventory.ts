...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const shop = verify(idToken, clientId, secret); const token = await exchange(shop, clientId, secret, idToken); const { inventoryItemId, locationId, delta, currentQuantity, reason = "correction", referenceDocumentUri } = req.body || {};
    if (typeof inventoryItemId !== "string" || typeof locationId !== "string" || !Number.isInteger(delta) || delta === 0 || !Number.isInteger(currentQuantity) || currentQuantity < 0) return json(res, 400, { error: "inventoryItemId, locationId, delta y currentQuantity son obligatorios." });
    ...
    const data = await graphql(shop, token, mutation, { input: { reason: String(reason), name: "available", referenceDocumentUri: typeof referenceDocumentUri === "string" ? referenceDocumentUri : `jatis-mutis://inventory/${Date.now()}`, changes: [{ delta, inventoryItemId, locationId, changeFromQuantity: currentQuantity }] }, idempotencyKey: randomUUID() });
