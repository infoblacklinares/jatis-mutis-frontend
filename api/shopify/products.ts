...
type ShopifyProduct = { id: string; title: string; handle: string; descriptionHtml: string; featuredImage: { url: string } | null; vendor: string; productType: string; tags: string[]; variants: { nodes: Array<{ id: string; sku: string | null; title: string; inventoryQuantity: number; price: string; compareAtPrice: string | null; inventoryItem: { id: string; measurement: { weight: { value: number; unit: string } | null } } | null; availableForSale: boolean }> } };
...
const query = `query Products($first: Int!, $after: String) { products(first: $first, after: $after, sortKey: TITLE) { nodes { id title handle descriptionHtml featuredImage { url } vendor productType tags variants(first: 100) { nodes { id sku title inventoryQuantity price compareAtPrice inventoryItem { id measurement { weight { value unit } } } availableForSale } } } pageInfo { hasNextPage endCursor } } }`;
